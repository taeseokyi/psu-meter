...
HUB_TOP = 3;
HUB_L   = 93; HUB_R = 103;
// ── 측정창과 증기 웰 (2026-09-02 신설) ────────────────────────────────
//  ★ 프리즘 상부를 **필요한 만큼만** 연다. 조명 패치가 8.6 mm, 빔의 두께 방향이
//    약 9 mm 이므로 14 x 12 면 충분하다. 전에는 50 x 17.7(전폭)을 열어 두었다.
//    줄이면 (1) 프리즘을 무는 살이 넓어지고 (2) 증기 웰이 작아진다.
...
//    닫힌 부피는 z 0~SEAT 뿐이라 0.15 cm³, 포화에 액적의 0.02 % (0.006 PSU).
WIN_HX  = 7;      // 창 반폭 x (바닥, z=0)
//  ★ 창은 **프리즘보다 좁아야** 한다. 프리즘 10 mm 에 포켓이 10.7 이라
//    프리즘 옆으로 편측 0.35 mm 틈이 있는데, 창이 그보다 넓으면 그 틈이
//    트레이에 그대로 드러나 액적이 모세관으로 빨려 들어간다.
//    창 반폭 4 면 프리즘 위에 편측 1 mm 의 살이 남아 그 틈을 **덮는다.**
WIN_HY  = 4;      // 창 반폭 y — 프리즘 반두께 5 보다 1 mm 안쪽
...
//  ★ 빔의 두께 방향 폭은 **젖은 영역 안**에 들어와야 한다. 창이 8 mm 이므로
//    9.5 로 두면 빔의 가장자리가 마른 프리즘 면을 때려 염도와 무관한 배경이 된다.
//    좁히면 광량이 26 % 줄지만 잡음 항은 0.0026 PSU 라 사실상 무해하다.
CH_Y   = 2*WIN_HY - 1;                           // 7     채널 폭(두께 방향)
BAF_T  = 1.2;                                    //       인쇄 조리개 두께
...
//   여기서 빼지 않고 따로 절삭한다. 창 밖에 남는 살이 프리즘을 위에서 물어 주므로
//   별도 누름쇠가 필요 없다 — 창을 14 x 12 로 줄이면서 무는 면적이 훨씬 넓어졌다.
module body_profile() {
...
module body_full() {
    union() {
        difference() {
            ...
            // 측정창 — 사면. 필요한 만큼만 열고 뚜껑이 자기정렬하게 한다.
...
module cap() {
    ...
    difference() {
        union() {
            ...
            // ── 증기 웰 보스 ────────────────────────────────────────────
            //  창 사면에 **아래 모서리로 선접촉**해 자기정렬하고 SEAT 만큼 뜬다.
            //  보스 사면(CAP_TAP 12°)이 창(WIN_TAP 25°)보다 완만해서 위로 갈수록
            //  벌어진다 — 접촉선은 딱 z=SEAT 한 줄이고 그 위는 안 닿는다.
            //  닫히는 부피가 z 0~SEAT 뿐이라 증기가 거의 즉시 포화돼 증발이 멎는다.
            taper_box(WIN_HX + SEAT*(tan(WIN_TAP) - tan(CAP_TAP)),
                      WIN_HY + SEAT*(tan(WIN_TAP) - tan(CAP_TAP)),
                      SEAT, COV_TOP, CAP_TAP);
        }
        // 속 — 아래로 열린다. 단 증기 웰 보스가 있는 자리는 남긴다.
        difference() {
...
// ============================================================================
BLADE_T = 0.5; BLADE_W = CH_Y;   // 날 길이 = 빔의 두께 방향 폭
...
//        · 64 mm² x=±18.5 z=+1.5 — 측정창 턱(프리즘을 무는 살). 두께 HUB_TOP=3
//          창을 14x8 로 줄이면서 이 살이 훨씬 넓어졌다 — 의도한 것이다
